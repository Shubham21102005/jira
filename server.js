...
app.get('/callback', async (req, res, next) => {
	try {
        ...
        const { data } = await axios.post(`${ATLASSIAN_AUTH_SERVER}/oauth/token`, body.toString(), {
...
async function getCloudId(req) {
	...
	const resp = await axios.get(`${ATLASSIAN_API}/oauth/token/accessible-resources`, {
		headers: { Authorization: `Bearer ${accessToken}` }
	});
	// Pick first Jira resource for simplicity
	const jiraSite = resp.data.find(r => r.scopes && r.scopes.length && r.id && r.url);
...
app.post('/issues', ensureAuth, async (req, res, next) => {
	try {
        ...
        let details = err && err.response && err.response.data ? JSON.stringify(err.response.data) : '';
        console.error('Create issue failed', details || err);
        try {
            const projects = await getProjects(req);
            res.status(400).render('new-issue', { projects, error: (details || err.message || 'Failed to create issue') });
        } catch (_) {
            res.status(400).render('new-issue', { projects: [], error: (details || err.message || 'Failed to create issue') });
        }
	}
...
app.use((err, req, res, next) => {
	console.error(err);
	res.status(500).render('error', { message: err.message || 'Unexpected error' });
});
